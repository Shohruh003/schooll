...
function EditAdminModal({ editAdminModal, setEditAdminModal }) {
  ...
  const changeOption = (evt) => {
    ...
    if (evetValue === "teacher") {
      ...
      elClass.disabled = false;
      elClass1.disabled = false;
    } else if (evetValue === "psychologist") {
      elParent.disabled = true;
      elEmail.disabled = false;
      elPassword.disabled = false;
      elPassword3.disabled = false;
  ...
  const hendlSend = (evt) => {
    ...
    if (password1.value !== password2.value) {
      return alert("Tasdiqlash paroli xato !");
    }
    ...
    formData.append(
      "full_name",
      user?.full_name ? user?.full_name : editUser?.full_name
    );
    formData.append(
      "birth_date",
      user?.birth_date ? user?.birth_date : editUser?.birth_date
    );
    formData.append(
      "password",
      user?.password ? user?.password : editUser?.password
    );
    formData.append(
      "confirm_password",
      user?.password ? user?.password : editUser?.password
    ...
    formData.append(
      ...
      (user?.pupil_class
        ? user?.pupil_class
        : editUser?.pupil_class === null
        ? ["1-A"]
        : editUser?.pupil_class[0]?.slice(0, 1)) +
        "-" +
        (user?.pupil_class_str
          ? user?.pupil_class_str
          : (editUser?.pupil_class &&
              editUser?.pupil_class[0]?.slice(2, 3)?.toUpperCase()) ||
            "")
    );
    ...
    formData.append(
      "is_morning",
      user?.shift ? user?.shift : editUser?.is_morning
    );
  ...
  const hendlParent = async (evt) => {
    ...
    await api
      .get("/users/users/?status=parents")
      .then((response) => {
  ...
  return (
    ...
    >
      <Modal.Body>
        <form
          onSubmit={(e) => hendlSend(e)}
          className="table table-striped modal_update"
        >
          <thead>
            <Modal.Title
              style={{ color: theme }}
          ...
          <tbody className="modal_add">
            ...
            <div className="input_box pass3">
              <label for="pass3" class="form-label">
                Пароль
              </label>
              <input
            ...
            <div className="input_box">
              ...
              <input
                defaultValue={editUser?.birth_date}
                onChange={(event) => {
                  setUser({
                    ...user,
            ...
            <div className="input_box pass5">
              <label for="pass5" class="form-label">
                Повторите пароль
              </label>
              <input
                defaultValue={editUser?.password}
              ...
              <img
                ...
                onClick={() => {
                  const password2 = document.querySelector("#pass5");
                  return password2.type === "password"
                    ? (password2.type = "text")
            ...
            <div className="input_box">
              <label for="7" class="form-label class-lable">
                Класс
              </label>
              <select
                defaultValue={
              ...
              <select
                ...
                className="class2"
              >
                <option defaultValue="А">
                  А<img src={selectIcon} alt="icon" />
                </option>
            ...
            <div className="input_box2">
              ...
              <div className="box2_item">
                <label class="form-label">Укажите смену</label>
                <br />
                <div className="d-flex align-items-center gap-2">
                  <input
                    onChange={(event) => {
