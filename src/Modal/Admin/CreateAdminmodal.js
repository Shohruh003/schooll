...
function CreateAdminModal({ adminModal, setAdminModal }) {
  ...
  const hendlSend = (evt) => {
    ...
    formData.append(
      "status",
      user?.status ? user?.status : (user.status = "pupil")
    );
    formData.append(
      "pupil_class",
      (user?.pupil_class ? user?.pupil_class : "1") +
        "-" +
        (user?.pupil_class_str ? user?.pupil_class_str : "A")
    );
          ...
          <tbody className="modal_add">
            ...
            <div className="input_box2">
              ...
              <div className="d-flex align-items-center check gap-2">
                <label for="6" class="form-label" role="button">
                  Добавить несколько
                </label>
                <input
                  type="checkbox"
